...
#if defined(NDEBUG)
# error "Youbercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbe42d06e079e3be6c32a076012fdac267e3cee423f05665a21bce94b687b9c22");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Youbercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Youbercoin Signed Message:\n";
...
{
    // Youbercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Youbercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Youbercoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Youbercoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Youbercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Youbercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbe42d06e079e3be6c32a076012fdac267e3cee423f05665a21bce94b687b9c22");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "creado por un español.Jose Ignacio";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x01b22814e53534b15f4ebfa4360b5f6ddce1eb148c9bacde31ce535b42a5954e"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Youbercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YoubercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YoubercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YoubercoinMiner(CWallet *pwallet)
{
    printf("YoubercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("youbercoin-miner");
    ...
    try { loop {
        ...
        printf("Running YoubercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YoubercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YoubercoinMiner, pwallet));
}
